...
class NewsSource(SQLModel, table=True):
    ...
    political_leaning: Optional[str] = Field(default=None, max_length=50)  # left, center, right
    bias_rating: Optional[str] = Field(default=None, max_length=50)  # left_bias, right_bias, mixed
    factual_reporting: Optional[str] = Field(default=None, max_length=50)  # very_high, high, mixed, low, very_low
    
    # Source Type
    source_type: str = Field(default="online", max_length=50)  # online, print, tv, radio, agency
    ...
    verified_by: Optional[uuid.UUID] = Field(default=None, foreign_key="users.id")
    
    # Content Guidelines
    content_policies: Optional[str] = Field(default=None, sa_column_kwargs={"type_": "TEXT"})
    editorial_standards: Optional[str] = Field(default=None, sa_column_kwargs={"type_": "TEXT"})
...
class NewsSourceFollow(SQLModel, table=True):
    ...
    __tablename__ = "news_source_follows"
    
    user_id: uuid.UUID = Field(foreign_key="users.id", primary_key=True)
    source_id: uuid.UUID = Field(foreign_key="news_sources.id", primary_key=True)
    created_at: datetime = Field(default_factory=datetime.utcnow)
